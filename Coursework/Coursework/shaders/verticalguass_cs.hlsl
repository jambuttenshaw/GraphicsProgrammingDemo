...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float texelSize = 1.0f / float(inDims.y);
    
    // Add the vertical pixels to the colour by the specific weight of each.
    colour += input.SampleLevel(samplerState, uv + float2(0.0f, texelSize * -2.0f), 0) * weight2;
    colour += input.SampleLevel(samplerState, uv + float2(0.0f, texelSize * -1.0f), 0) * weight1;
